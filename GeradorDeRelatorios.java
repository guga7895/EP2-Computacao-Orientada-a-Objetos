...
public class GeradorDeRelatorios {
	...
	private List<ProdutoFormatado> produtos;
	...
	public GeradorDeRelatorios(List<ProdutoFormatado> produtos, String algoritmo, String criterio, String filtro, String argFiltro, String argFiltro2){
	...
	private void ordena(int ini, int fim){
		MetodoDeOrdenacao OrdenacaoEscolhida = EscolhendoAlgoritmoOrdenacao.escolherAlgoritmoOrdenacao(algoritmo);
		OrdenacaoEscolhida.ordenar(ini,fim,this.produtos,criterio);
	}
	...
	public void geraRelatorio(String arquivoSaida) throws IOException {
		...
		for(int i = 0; i < produtos.size(); i++){
			ProdutoFormatado p = produtos.get(i);
			boolean selecionado = false;
			selecionado = Filtrador.escolherFiltracao(filtro, p, argFiltro, argFiltro2);
			if(selecionado){
				Formatador.formatar(p, out);
				count++;
			}
		}
	...
	public static List<ProdutoFormatado> carregaProdutos() {
		List<ProdutoFormatado> prod = new ArrayList<>();
		try {
			...
			while(in.hasNextLine()){
					...
					in.useDelimiter("\n");
					String cor = in.next();
					ProdutoFormatado produto = new ProdutoFormatado(id, desc, categ, qtd_estoque, preco, italico, negrito, cor);
					prod.add(produto);
				in.nextLine();
			}
		} catch(IOException e){
	...
	public static void main(String [] args) {
		...
		if(args.length < 4){
			...
			System.out.println("\tparâmetro de filtragem 2: outros possíveis argumentos adicionais necessários para a filtragem");
			System.out.println();
			System.exit(1);
		}
